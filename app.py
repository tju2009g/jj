...
@st.cache_data
def get_topic_of_day(day_of_year):
    topics = load_json("topic_ideas.json")
    return topics[day_of_year % len(topics)]


@st.cache_data
def get_focus_tip_of_day(day_of_year):
    tips = load_json("focus_tips.json")
...
st.markdown(
    ...
    .subject-social { background: #fee2e2; color: #991b1b; }
    .subject-math { background: #e0e7ff; color: #3730a3; }
    .subject-title {
        font-size: 1.1rem;
        font-weight: 800;
        color: #0f172a;
...
# 지현 맞춤 세특 · 탐구활동 도움
# ------------------------------------------------------------------
st.subheader("🎯 지현 맞춤 세특 · 탐구활동 도움")

with st.expander("🎓 미디어영상학과 관련 추천 선택과목 (참고용)"):
    ...
    with course_col3:
        st.markdown("**융합선택**")
        st.markdown("- 매체 의사소통\n- 소프트웨어와 생활")
    st.caption("출처: 커리어넷 전공 계열 분류 기준 (미디어영상학과와 가장 유사한 전공 기준 안내)")

topic_col, focus_col = st.columns(2)

topic = get_topic_of_day(DAY_OF_YEAR)
subject_class = {
    "국어": "subject-kor",
    "영어": "subject-eng",
    "과학": "subject-sci",
    "사회": "subject-social",
    "수학": "subject-math",
}.get(topic["subject"], "subject-kor")

with topic_col:
    topic_html = (
        '<div class="card"><div class="subject-badge ' + subject_class + '">' + topic["subject"] + " · 탐구주제</div>"
        f'<div style="color:#1f2937; font-weight:600; margin-top:0.3rem; line-height:1.5;">{topic["topic"]}</div>'
        '<div style="margin-top:0.6rem; color:#9ca3af; font-size:0.8rem;">미디어영상학과 계열 탐구활동 추천 리스트에서 오늘 골라온 주제예요.</div>'
        "</div>"
    )
    st.markdown(topic_html, unsafe_allow_html=True)

with focus_col:
    tip = get_focus_tip_of_day(DAY_OF_YEAR)
    growth_html = (
        '<div class="growth-box">'
        f'<div style="font-weight:700; color:#065f46; margin-bottom:0.4rem;">🌱 오늘의 성장 포인트 · {tip["area"]}</div>'
        f'<div style="color:#374151; line-height:1.5;">{tip["tip"]}</div>'
        "</div>"
    )
