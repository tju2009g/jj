...
@st.cache_data(ttl=24 * 60 * 60)
def fetch_news_section(keywords, limit):
...
with left:
    ...
    admission_news = fetch_news_section(ADMISSION_KEYWORDS, NEWS_PER_SECTION)
    render_news_cards(admission_news, "대입 제도 관련 뉴스를 불러오지 못했어요.")

    st.subheader("🎓 수도권·서울 영문학과 · 미디어커뮤니케이션 전공 소식")
    major_news = fetch_news_section(MAJOR_KEYWORDS, NEWS_PER_SECTION)
